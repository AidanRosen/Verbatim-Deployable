...
@app.post("/test")
async def postAudio(file: bytes = File(...)):
    model, df_state, _ = init_df()
    decoded_audio = decode(file, nchannels=1, sample_rate=16000, output_format=SampleFormat.SIGNED32)
    # audio, samplerate = torchaudio.load(buffer_)
    audio = torch.FloatTensor(decoded_audio.samples)
    samplerate = 16000
    # return Response(content=file, media_type="audio/wav")
    enhanced = enhance(model, df_state, audio)
    audio_tensor = post_process(enhanced)
    model = whisper.load_model("base.en")
    buffer_: bytes = io.BytesIO()
...
async def transcribeAudio(file:UploadFile = File(...)):
    ...
    print(file.filename)
    model = whisper.load_model("base.en")
    # audio, _ = torchaudio.load(file.file)
    result = model.transcribe("enhanced.wav")
